...
import FileBrowserModal from '../components/modals/FileBrowserModal' // Adjust import path
    ...
    return (
        <div className="h-full flex flex-col relative w-full overflow-hidden">
            ...
            <div className="flex-1 flex overflow-hidden">
                {/* Vertical Sidebar */}
                <div className="w-64 bg-dark-900/30 border-r border-dark-700/50 flex flex-col overflow-y-auto custom-scrollbar py-6 px-4 gap-2">
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id as TabId)}
                            className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-left group ${activeTab === tab.id
                                ? 'bg-primary-600 text-white shadow-lg shadow-primary-600/20'
                                : 'text-dark-400 hover:text-dark-100 hover:bg-dark-800/40'
                                }`}
                        >
                            <tab.icon size={20} className={activeTab === tab.id ? 'text-white' : 'text-dark-500 group-hover:text-dark-300'} />
                                ...
                                >
                                    ...
                                    {activeTab === 'about' && (
                                        <div className="space-y-12 py-4">
                                            ...
                                            <div className="pt-8">
                                                <button
                                                    onClick={() => window.electronAPI.app.openExternal('https://github.com/ReaQwQ/ReaLauncher/releases')}
                                                    className="w-full bg-primary-600/10 hover:bg-primary-600/20 text-primary-400 font-bold py-4 px-6 rounded-2xl border border-primary-600/20 hover:border-primary-600/40 transition-all flex items-center justify-center gap-3 group"
                                                >
                                                    <RefreshCw size={18} className="group-hover:rotate-180 transition-transform duration-500" />
                                                    <span className="tracking-wide uppercase text-sm">{t('settings.about.check_update')}</span>
                                                </button>
                                            </div>
